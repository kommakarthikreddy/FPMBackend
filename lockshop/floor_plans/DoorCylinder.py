...
from .models import DoorCylinder

@session_wrap()
@SkelRes()
def PostDoorCylinder (session, *args, **kwargs):
    ...
    door_hinge_data = DoorCylinder.create(session=session, name=name)
    session.commit()
...
@session_wrap()
@SkelRes()
def GetDoorCylinder (session, *args, **kwargs):
    ...
    door_type_data = DoorCylinder.get_by_id(session=session, id=int(id))
