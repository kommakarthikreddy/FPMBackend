...
from .models import DoorThreshold

@session_wrap()
@SkelRes()
def PostDoorThreshold(session, *args, **kwargs):
    ...
    door_hinge_data = DoorThreshold.create(session=session, type=type)
    session.commit()
...
@session_wrap()
@SkelRes()
def GetDoorThreshold(session, *args, **kwargs):
    ...
    door_type_data = DoorThreshold.get_by_id(session=session, id=int(id))
