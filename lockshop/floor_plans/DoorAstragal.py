...
from .models import DoorAstragal

@session_wrap()
@SkelRes()
def PostDoorAstragal (session, *args, **kwargs):
    ...
    door_hinge_data = DoorAstragal.create(session=session, type=type)
    session.commit()
...
@session_wrap()
@SkelRes()
def GetDoorAstragal (session, *args, **kwargs):
    ...
    door_type_data = DoorAstragal.get_by_id(session=session, id=int(id))
