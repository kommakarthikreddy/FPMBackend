...
from .models import DoorFlushBolt

@session_wrap()
@SkelRes()
def PostDoorFlushBolt (session, *args, **kwargs):
    ...
    door_hinge_data = DoorFlushBolt.create(session=session, type=type)
    session.commit()
...
@session_wrap()
@SkelRes()
def GetDoorFlushBolt (session, *args, **kwargs):
    ...
    door_type_data = DoorFlushBolt.get_by_id(session=session, id=int(id))
