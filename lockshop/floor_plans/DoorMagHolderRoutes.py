...
from .models import DoorMagHolder

@session_wrap()
@SkelRes()
def PostDoorMagHolder (session, *args, **kwargs):
    ...
    door_hinge_data = DoorMagHolder.create(session=session, type=type)
    session.commit()
...
@session_wrap()
@SkelRes()
def GetDoorMagHolder (session, *args, **kwargs):
    ...
    door_type_data = DoorMagHolder.get_by_id(session=session, id=int(id))
