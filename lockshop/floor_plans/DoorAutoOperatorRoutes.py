...
from .models import DoorAutoOperator

@session_wrap()
@SkelRes()
def PostDoorAutoOperator (session, *args, **kwargs):
    ...
    door_hinge_data = DoorAutoOperator.create(session=session, name=name)
    session.commit()
...
@session_wrap()
@SkelRes()
def GetDoorAutoOperator (session, *args, **kwargs):
    ...
    door_type_data = DoorAutoOperator.get_by_id(session=session, id=int(id))
