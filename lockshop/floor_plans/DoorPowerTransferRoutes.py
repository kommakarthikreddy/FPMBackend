...
from .models import DoorPowerTransfer

@session_wrap()
@SkelRes()
def PostDoorPowerTransfer(session, *args, **kwargs):
    ...
    door_hinge_data = DoorPowerTransfer.create(session=session, type=type)
    session.commit()
...
@session_wrap()
@SkelRes()
def GetDoorPowerTransfer(session, *args, **kwargs):
    ...
    door_type_data = DoorPowerTransfer.get_by_id(session=session, id=int(id))
