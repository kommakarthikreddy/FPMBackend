...
from .models import DoorExitDevice

@session_wrap()
@SkelRes()
def PostDoorExitDevice(session, *args, **kwargs):
    ...
    door_hinge_data = DoorExitDevice.create(session=session, name=name)
    session.commit()
...
@session_wrap()
@SkelRes()
def GetDoorExitDevice(session, *args, **kwargs):
    ...
    door_type_data = DoorExitDevice.get_by_id(session=session, id=int(id))
