...
from .models import DoorStrike

@session_wrap()
@SkelRes()
def PostDoorStrike(session, *args, **kwargs):
    ...
    door_hinge_data = DoorStrike.create(session=session, type=type)
    session.commit()
...
@session_wrap()
@SkelRes()
def GetDoorStrike(session, *args, **kwargs):
    ...
    door_type_data = DoorStrike.get_by_id(session=session, id=int(id))
