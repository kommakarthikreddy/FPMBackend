...
from .models import DoorAutoDrBtm

@session_wrap()
@SkelRes()
def PostDoorAutoDrBtm(session, *args, **kwargs):
    ...
    door_hinge_data = DoorAutoDrBtm.create(session=session, type=type)
    session.commit()
...
@session_wrap()
@SkelRes()
def GetDoorAutoDrBtm(session, *args, **kwargs):
    ...
    door_type_data = DoorAutoDrBtm.get_by_id(session=session, id=int(id))
