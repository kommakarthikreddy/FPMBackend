...
from .models import DoorPivot

@session_wrap()
@SkelRes()
def PostDoorPivot(session, *args, **kwargs):
    ...
    door_hinge_data = DoorPivot.create(session=session, type=type)
    session.commit()
...
@session_wrap()
@SkelRes()
def GetDoorPivot(session, *args, **kwargs):
    ...
    door_type_data = DoorPivot.get_by_id(session=session, id=int(id))
