...
from .models import DoorDelayEgress

@session_wrap()
@SkelRes()
def PostDelayEgress(session, *args, **kwargs):
    ...
    door_hinge_data = DoorDelayEgress.create(session=session, type=type)
    session.commit()
...
@session_wrap()
@SkelRes()
def GetDoorDelayEgress(session, *args, **kwargs):
    ...
    door_type_data = DoorDelayEgress.get_by_id(session=session, id=int(id))
