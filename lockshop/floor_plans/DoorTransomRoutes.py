...
from .models import DoorTransom

@session_wrap()
@SkelRes()
def PostDoorTransom(session, *args, **kwargs):
    ...
    door_transom_data = DoorTransom.create(session=session, material=material)
    session.commit()

    return {
        "_code": 200,
        "message": "Success",
        "door_transom": door_transom_data.sanitize()
    }


@session_wrap()
@SkelRes()
def GetDoorTransom(session, *args, **kwargs):
    ...
    door_type_data = DoorTransom.get_by_id(session=session, id=int(id))
