...
from .models import DoorCoordinator

@session_wrap()
@SkelRes()
def PostDoorCoordinator (session, *args, **kwargs):
    ...
    door_hinge_data = DoorCoordinator.create(session=session, type=type)
    session.commit()
...
@session_wrap()
@SkelRes()
def GetDoorCoordinator (session, *args, **kwargs):
    ...
    door_type_data = DoorCoordinator.get_by_id(session=session, id=int(id))
