...
@session_wrap()
@SkelRes()
def PostDoorLockSet(session, *args, **kwargs):
    ...
    door_hinge_data = DoorLockset.create(session=session, type=type)
    session.commit()
...
@session_wrap()
@SkelRes()
def GetDoorLockSet(session, *args, **kwargs):
    ...
    door_type_data = DoorLockset.get_by_id(session=session, id=int(id))
