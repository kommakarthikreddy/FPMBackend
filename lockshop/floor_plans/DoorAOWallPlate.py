...
from .models import DoorAOWallPlate

@session_wrap()
@SkelRes()
def PostDoorAOWallPlate (session, *args, **kwargs):
    ...
    door_hinge_data = DoorAOWallPlate.create(session=session, type=type)
    session.commit()
...
@session_wrap()
@SkelRes()
def GetDoorAOWallPlate (session, *args, **kwargs):
    ...
    door_type_data = DoorAOWallPlate.get_by_id(session=session, id=int(id))
