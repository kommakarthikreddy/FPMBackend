...
from .models import DoorCloser

@session_wrap()
@SkelRes()
def PostDoorCloser (session, *args, **kwargs):
    ...
    door_hinge_data = DoorCloser.create(session=session, name=name)
    session.commit()
...
@session_wrap()
@SkelRes()
def GetDoorCloser (session, *args, **kwargs):
    ...
    door_type_data = DoorCloser.get_by_id(session=session, id=int(id))
