...
from .models import DoorSeal

@session_wrap()
@SkelRes()
def PostDoorSeal (session, *args, **kwargs):
    ...
    door_hinge_data = DoorSeal.create(session=session, type=type)
    session.commit()
...
@session_wrap()
@SkelRes()
def GetDoorSeal (session, *args, **kwargs):
    ...
    door_type_data = DoorSeal.get_by_id(session=session, id=int(id))
