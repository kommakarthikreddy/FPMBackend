...
from .models import DoorTrim

@session_wrap()
@SkelRes()
def PostDoorTrim(session, *args, **kwargs):
    ...
    door_hinge_data = DoorTrim.create(session=session, name=name)
    session.commit()
...
@session_wrap()
@SkelRes()
def GetDoorTrim(session, *args, **kwargs):
    ...
    door_type_data = DoorTrim.get_by_id(session=session, id=int(id))
