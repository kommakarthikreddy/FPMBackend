...
from .models import DoorStop

@session_wrap()
@SkelRes()
def PostDoorStop (session, *args, **kwargs):
    ...
    door_hinge_data = DoorStop.create(session=session, type=type)
    session.commit()
...
@session_wrap()
@SkelRes()
def GetDoorStop (session, *args, **kwargs):
    ...
    door_type_data = DoorStop.get_by_id(session=session, id=int(id))
