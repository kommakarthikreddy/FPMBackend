...
from .models import DoorSweep

@session_wrap()
@SkelRes()
def PostDoorSweep (session, *args, **kwargs):
    ...
    door_hinge_data = DoorSweep.create(session=session, name=name)
    session.commit()
...
@session_wrap()
@SkelRes()
def GetDoorSweep (session, *args, **kwargs):
    ...
    door_type_data = DoorSweep.get_by_id(session=session, id=int(id))
